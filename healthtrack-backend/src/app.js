...
// 导入路由
const { authRoutes, healthRoutes, publicRoutes, personalizedHealthRoutes } = require('./routes');
  ...
  res.json({
    ...
    endpoints: {
      ...
      personalizedHealth: {
        ...
        'DELETE /api/health-profile': '删除个性化健康档案 [需认证]',
        'GET /api/health-profile/standards': '获取个性化健康标准 [需认证]',
...
async function startServer() {
  ...
  if (!dbConnected) {
    console.warn('⚠️  数据库连接失败，将以开发模式继续运行');
  }
